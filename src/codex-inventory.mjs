...
function sourceIdFor(path) {
  return hash(`codex-source:${normalizePathForIdentity(path)}`);
}

function hashedRelation(kind, value) {
...
async function readFirstLine(path, maxBytes) {
  ...
  for await (const chunk of createReadStream(path, {
    ...
    if (newline >= 0) {
      ...
      return {
        ...
        metadataHash: hash(line)
      };
    }
  }

  throw new Error('The first JSONL record exceeds the configured metadata limit.');
}

function metadataFromFirstLine(firstLine, sourceId) {
  const row = parseJsonLine(firstLine);
  const payload = row?.type === 'session_meta' && row.payload
    ? row.payload
    : {};
  const sessionId = payload.session_id ?? payload.id ?? sourceId;
  const repository = payload.git?.repository_url;
  const branch = payload.git?.branch;
  const relations = [
    hashedRelation('workspace', payload.cwd),
    hashedRelation('repository', repository),
    hashedRelation('branch', repository && branch ? `${repository}:${branch}` : branch),
    hashedRelation('parent', payload.forked_from_id)
  ].filter(Boolean);

  return {
    sessionIdHash: hash(`session:${sessionId}`),
    parentSessionIdHash: payload.forked_from_id
      ? hash(`session:${payload.forked_from_id}`)
      : null,
    relationKeys: [...new Set(relations)].sort()
  };
}

export async function readSourceIdentity(source) {
  const path = resolve(source);
  const sourceId = sourceIdFor(path);
  const first = await readFirstLine(path, DEFAULTS.firstLineMaxBytes);
  const metadata = metadataFromFirstLine(first.line, sourceId);
  ...
  for await (const chunk of stream) {
    ...
    while ((newline = buffer.indexOf(0x0a)) >= 0) {
      ...
      if (!row) {
        ...
        addCount(recordCounts, row.type);
        if (typeof row.timestamp === 'string') {
          firstEventAt ??= row.timestamp;
          lastEventAt = row.timestamp;
        }
        if (row.type === 'event_msg' && LIFECYCLE_EVENTS.has(row.payload?.type)) {
          lastLifecycle = row.payload.type;
        }
      }
...
function classifyThread({
  ...
  let state;
  if (lastLifecycle === 'task_complete') {
    state = 'completed';
  } else if (lastLifecycle === 'turn_aborted') {
    state = 'aborted';
  } else if (lastLifecycle === 'task_started') {
    state = 'active';
  } else {
  ...
  if (grew && ['completed', 'aborted'].includes(previousState) && state === 'active') {
    return 'reopened';
  }
...
function expiryFor(state, now, peerTtlSeconds) {
...
function loadCheckpoint(path) {
  ...
  if (checkpoint.schemaVersion !== 1 || checkpoint.provider !== 'codex') {
    throw new Error('Unsupported Codex inventory checkpoint.');
  }
  ...
  while (descriptor === undefined) {
    try {
      ...
      if (Date.now() - startedAt >= options.lockTimeoutMs) {
        throw new Error('Codex inventory checkpoint is busy.');
      }
...
export async function planInventory(inputOptions) {
  ...
  return {
    ...
    skippedByFileLimit: discovered.overflow.map(({ path }) => sourceIdFor(path)),
    limits: {
...
export async function readRelatedDeltas(inputOptions) {
  const options = {
    maxLedgerFiles: 100,
    afterSequence: 0,
    includeSelf: false,
  ...
  const sourceId = sourceIdFor(options.source);
  const first = await readFirstLine(options.source, DEFAULTS.firstLineMaxBytes);
  const metadata = metadataFromFirstLine(first.line, sourceId);
  ...
  for (const file of ledger.selected) {
    ...
    for (const line of text.split(/\r?\n/u)) {
      ...
      if (!delta || delta.provider !== 'codex' || !Number.isInteger(delta.sequence)) {
        continue;
      }
...
async function runInventoryLocked(options) {
  const checkpoint = loadCheckpoint(options.checkpoint);
  const discovered = await discoverSources(options.source, options);
  ...
  for (const candidate of discovered.selected) {
    const sourceId = sourceIdFor(candidate.path);
    try {
      const first = await readFirstLine(candidate.path, options.firstLineMaxBytes);
      const metadata = metadataFromFirstLine(first.line, sourceId);
      const previous = checkpoint.sources[sourceId];
      const reset = Boolean(previous) && (
        candidate.stat.size < previous.nextOffset ||
        first.metadataHash !== previous.metadataHash
      );
      ...
      const lastLifecycle = segment.lastLifecycle ?? previous?.lastLifecycle ?? null;
      ...
      const classification = ['completed', 'aborted'].includes(threadState)
        ? 'candidate'
        : 'unverified';
      const deltaCore = [
        sourceId,
      ...
      deltas.push({
        ...
        sourceId,
        observedAt: nowIso,
        expiresAt: expiryFor(threadState, options.now, options.peerTtlSeconds),
        classification,
        threadState,
  ...
  checkpoint.adapterVersion = ADAPTER_VERSION;
...
export async function runInventory(inputOptions) {
  const options = normalizedOptions(inputOptions);
